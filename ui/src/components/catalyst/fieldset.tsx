...
export function Label({ className, ...props }: { className?: string } & Omit<Headless.LabelProps, 'as' | 'className'>) {
  return (
    <Headless.Label
      data-slot="label"
      {...props}
      className={clsx(className, 'text-base/6 text-zinc-950 data-disabled:opacity-50 sm:text-sm/6 dark:text-white')}
    />
  );
}
...
export function Description({
  ...
  return (
    <Headless.Description
      ...
      {...props}
      className={clsx(className, 'text-base/6 text-zinc-600 data-disabled:opacity-50 sm:text-sm/6 dark:text-zinc-400')}
    />
  );
}
