...
function MultiHostGpuMonitor({ hosts }: { hosts: HostInfo[] }) {
  ...
  const content = useMemo(() => {
    ...
    if (allGpus.length === 0) {
      return (
        <div
          className="bg-yellow-50 border border-yellow-300 text-yellow-800 dark:bg-yellow-900 dark:border-yellow-700 dark:text-yellow-300 px-4 py-3 rounded relative"
          role="alert"
        >
...
function LocalGpuMonitor() {
  ...
  const content = useMemo(() => {
    ...
    if (error) {
      return (
        <div
          className="bg-red-50 border border-red-300 text-red-800 dark:bg-red-900 dark:border-red-600 dark:text-red-200 px-4 py-3 rounded relative"
          role="alert"
        >
